/*
  This is almost identical to fib.cpp except adding "fib by matrix (log(N) loop)".
  The new method uses a loop instead of recursion to calculate the power in O(log(N)) time.
*/
#include <iostream>
#include <chrono>
...
long long fib_by_matrix(Mode mode, int n) {
  if (n <= 1)
    return 1;
...
int main(int argc, char **argv) {
  ...
  auto fm = [](int n) { return fib_by_matrix(Mode::kLoop, n); };
  auto fm2 = [](int n) { return fib_by_matrix(Mode::kRecursion, n); };
  auto fm3 = [](int n) { return fib_by_matrix(Mode::kFastLoop, n); };

  time_it("fib           (loop)            ", fib, n, k);
  time_it("fib by matrix (loop)            ", fm, n, k);
  time_it("fib by matrix (log(N) recursion)", fm2, n, k);
  time_it("fib by matrix (log(N) loop)     ", fm3, n, k);
